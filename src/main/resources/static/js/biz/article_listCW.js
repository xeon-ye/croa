// var htFlag = user.companyCode == "HTLS" ? false : true;
...
var Business = {
    ...
    getDeptId:function(){
        ...
        if(deptCompanyCode == "JT" && (deptCode == "XZ" || deptCode == "RS" || deptCode == "CW" || user.currentCompanyQx || deptCode == "GL")){
        ...
        }else if(deptCode == "XZ" || deptCode == "RS" || deptCode == "CW" || user.currentCompanyQx || deptCode == "GL"){
    ...
    exportAll: function () {
        layer.msg("正在处理中，请稍候。", {time: 5000, shade: [0.7, '#393D49']});
        var params =  removeBlank($("#searchForm").serializeJson());
        params["right"] = 1;
        location.href = "/article/exportArticleFGS?" + $.param(params);
    },
    //设置统计数据
    setArticleResult: function () {
        $("#tj").find(".text-danger").html(0);
        $.ajax({
            url: baseUrl + "/article/articleResultFGS",
...
var gridObject = {
    url: baseUrl + '/article/articleListFGS',
