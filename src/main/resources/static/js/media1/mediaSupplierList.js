...
function initMediaType() {
	requestData(null,"/mediaPlate/userId","get","json",false, function (data) {
		...
		requestData(null,"/mediaAudit/getMediaSupplierNumbers/0","get","json",false,function (mediaTotal) {
			var mediaTotalMap = {};
...
function createTable() {
	...
	$tableMedias.jqGrid({//2600
		url: baseUrl + '/mediaAudit/listMediaSupplier',
		datatype: "json",
		postData: termForm,
		mtype: 'get',
		altRows: true,
		...
		colModel: [
			...
			},
			{
				...
				formatter: function (v, options, row) {
					if(row.mediaPriceCellList && row.mediaPriceCellList.length > 0){
						return row.mediaPriceCellList[0].supplierId;
					}
				}
			},
			{
			...
			},
			{
				...
				hidden: false,
				formatter: function (v, options, row) {
					var html = "<a class='text-success' onclick='mediaSupplierChangeObj.mediaSupplierChangeShow("+row.relateId+","+row.userId+");'>异动详情&nbsp;</a>";
					if(row.mediaPriceCellList && row.mediaPriceCellList.length > 0){
						//媒体和媒体关系都审核通过，祥和部长和组长可以进行拷贝
						// if(currentDeptQx && companyCode == 'XH' && row.companyCode != 'XH' && row.state == 1 && row.mediaPriceCellList[0].supplierRelateState == 1){
						// 	html += "&nbsp;<a class='text-success' onclick='copyMedia(\"copyMediaRelate\"," + row.relateId + ")'>拷贝</a>";
						// }
						//仅有责任人自己可以操作启用停用
						if( user.id == row.userId && row.mediaPriceCellList[0].supplierRelateState == 1){
							if(row.mediaPriceCellList[0].enabled == 0){
								html += "&nbsp;<a class='text-success' onclick='opMedia(\"stopRelate\"," + row.id + ","+ row.relateId +",\""+row.companyCode+"\""+","+row.userId+")'>停用</a>";
							}else{
								html += "&nbsp;<a class='text-success' onclick='opMedia(\"activeRelate\"," + row.id +  ","+ row.relateId +",\""+row.companyCode+"\""+","+row.userId+")'>启用</a>";
							}
						}
					}
					return html;
		...
		/*ondblClickRow: function (rowid, iRow, iCol, e) {
			...
			//不能修改分公司的媒体，普通媒介、组长或部长可以修改同板块所有
			if(rowData.companyCode == companyCode){
				page('/media1/mediaEdit?id=' + rowData.id + "&mediaPlateId="+$("#plateId").val() + "&mediaPlateName="+rowData.mediaName, '媒体修改');
			}
		},*/
...
//启用和停用媒体，这里需要改成启用/停用媒体供应商关系
function opMedia(type, mediaId, relateId, mediaCompanyCode, userId) {
	var opName = type == "activeRelate" ? "启用" : "停用"; //操作名称
	var errorInfo = "";
	if(user.id != userId){
	...
	requestData({mediaId:mediaId, relateId:relateId, standarPlatformFlag: $("#standarPlatformFlag").val() || 0},"/mediaAudit/" + type , "get", "json", true,function (data) {
		if (data.code == 200) {
			layer.msg("成功");
			reflushTable();//刷新表格
			triggerPageBtnClick("/media1/mediaSupplierAuditList","mediaListSearchBtn"); //触发媒体关系审核Tab刷新
...
function batchDelete(obj) {
	var url = "/mediaAudit/deleteBatchRelate";
	var tips = "确认批量删除?";
	...
	var mediaSupplierRelates = new Array(); //媒体关系
	if (url.length > 0 && ids.length > 0) {
		...
		$(ids).each(function (index, id) {
			...
			if(row.userId != user.id){
				info.push("<span style=\"color: brown;font-size: 12px;\">很抱歉，存在["+row.name+"]不能删除，只有该媒体关系的责任人才能删除！</span>")
		...
		layer.confirm(tips, {
			...
			if(relateIds.length > 0){
				var param = {mediaIds:mediaIds, relateIds: relateIds, mediaNames: mediaNames, userIds: userIds ,mediaSupplierRelates:mediaSupplierRelates, standarPlatformFlag: $("#standarPlatformFlag").val() || 0};
				requestData(param, baseUrl + url, "get","json",true,function (data) {
					...
					reflushTable();
					triggerPageBtnClick("/media1/mediaList","mediaListSearchBtn"); //触发媒体管理Tab刷新
					triggerPageBtnClick("/media1/mediaSupplierAuditList","mediaListSearchBtn"); //触发媒体管理Tab刷新
				});
			}else{
				var text = "";
				if(info.length > 0){
	...
	if(ids && ids.length > 0){
		...
		$(ids).each(function (index, id) {
			...
			if(row.companyCode != companyCode){
				...
				relateIds.push(row.id+"-"+row.supplierId);
				mediaIds.push(row.id)
			}
		});
		if(info.length > 0){
		...
		var json = {};
		json.mediaIds = mediaIds;
		json.relateIds = relateIds;
		json.plateId = $("#plateId").val();
		json.standarPlatformFlag = $("#standarPlatformFlag").val();
		json.plateName = $("#mediaTypeText").attr("title");
		location.href = baseUrl+"/mediaAudit/batchChooseRelateExport?"+$.param(json);
	}else{
		swal("请选择要操作的数据!");
	}
