...
function initMediaType() {
	requestData(null,"/mediaPlate/userId","get","json",false, function (data) {
		...
		requestData(null,"/mediaAudit/getMediaNumbers/1","get","json",false,function (mediaTotal) {
			var mediaTotalMap = {};
...
function createTable() {
	...
	$tableMedias.jqGrid({//2600
		url: baseUrl + '/mediaAudit/listAuditMedia',
		datatype: "json",
		postData: termForm,
		mtype: 'get',
		altRows: true,
		...
		colModel: [
            ...
            },
			{
				...
				formatter: function (v, options, row) {
					var html = "";
					html +=  "<a class='text-success' onclick='audit(\"pass\"," + row.id + ",\""+row.companyCode+"\")'>通过</a>&nbsp;<a class='text-danger' onclick='audit(\"reject\"," + row.id + ",\""+row.companyCode+"\")'>驳回</a>";
					html += "&nbsp;<a class='text-success' onclick='lookSupplierPrice(" + row.id + ", \""+row.name+"\")'>价格详情</a>";
		...
		loadComplete: function (data) {
			...
			// if (href.lastIndexOf("media/add") > -1) {
			// var rowData = $(this).jqGrid("getRowData", rowid);
			// editMedia(rowid, rowData);
			// }
		},
		...
		switch (data.type) {
				...
				if(data.dbJson){  //如果dbJson字段有值则使用，否为dbsql有值
					...
					if (sqlDatas && sqlDatas.length > 0) {  //如果对象存在，并且个数大于0
						$.each(sqlDatas,function (i,item) {
							dbHtml += "<span  class='"+dataClass+" col-md-1 i-checks' title='"+item.name+"'><input id=\"${name}\" name=\"${name}\" cell-name=\"${cell-name}\" type='"+data.type+"' value='" + item.id + "' /> " + item.name + "</span>";
						});
					}
				}
				html += dbHtml;
			...
			case 'select':
				...
				if(data.dbJson){  //如果dbJson字段有值则使用，否为dbsql有值
					...
					if (sqlDatas && sqlDatas.length > 0) {  //如果对象存在，并且个数大于0
						$.each(sqlDatas,function (i,item) {
							dbHtml += '<span class="col-md-1" title="'+item.name+'"><span class="" style="padding: 5px" onclick="loadMedia(\''+cell+'\','+item.id+',this)">'+item.name+'</span></span>';
						})
					}
				}
				html += dbHtml;
...
//审核操作
function audit(type, id, mediaCompanyCode) {
	var companyCode = user.dept.companyCode; //当前用户公司代码
	var opName = type == "pass" ? "审核通过" : "驳回"; //操作名称
	...
	requestData(null,"/mediaAudit/" + type + "/" + id, "get", "json", true,function (data) {
		if (data.code == 200) {
			...
			// var plateId = $("#plateId").val();
			// var $plateNode = $("#mediaPlate span[data-value='"+plateId+"']");
			// var auditNum = parseInt($plateNode.attr("data-mcount"))-1;
			// $plateNode.attr("data-mcount",auditNum);
			// $plateNode.text($plateNode.attr("title")+"("+auditNum+")");
...
function auditOprate(obj, flag) {
	...
	// 通过；
	if (flag == 0) {
		url = "/mediaAudit/passBatch";
		tips = "确认批量通过?";
		opName = "审核通过";
	}
	// 驳回；
	if (flag == 1) {
		url = "/mediaAudit/rejectBatch";
		tips = "确认批量驳回?";
		opName = "驳回";
	}
	// 删除；
	if (flag == 2) {
		url = "/mediaAudit/deleteBatch";
		tips = "确认批量删除?";
		opName = "删除";
	}
	...
	if (url.length > 0 && ids.length > 0) {
		...
		layer.confirm(tips, {
			...
			if(ids.length > 0){
				var param = { ids: ids, mediaNames: mediaNames, userIds: userIds };
				requestData(param, baseUrl + url, "get","json",true,function (data) {
					...
					if (data.code == 200) {
						...
						// var plateId = $("#plateId").val();
						// var $plateNode = $("#mediaPlate span[data-value='"+plateId+"']");
						// var auditNum = parseInt($plateNode.attr("data-mcount"))-ids.length;
						// $plateNode.attr("data-mcount",auditNum);
						// $plateNode.text($plateNode.attr("title")+"("+auditNum+")");
					} else {
						if (getResCode(data)) {
...
var supplierPriceTableObj = {
	...
	supplierPriceTable: {
		...
		sortable: true,
		shrinkToFit: true,
		// colNames: ['媒体ID','媒体公司代码','媒体名称','供应商','供应商联系人','价格','是否拷贝','拷贝备注'],//表头
