...
var mediaChangePermissionFlag = false;//媒体异动访问权限， 默认没有
$(function () {
    ...
    //获取媒体异动访问权限
    requestData(null, "/mediaAudit/getMediaChangePermission", "post", "json", false, function (data) {
        if(data.code == 200){
            mediaChangePermissionFlag = true;
            $("#mediaChooseChangeExport").css("display", "inline-block");
            $("#mediaChangeExport").css("display", "inline-block");
        }else {
            mediaChangePermissionFlag = false;
            $("#mediaChooseChangeExport").css("display", "none");
            $("#mediaChangeExport").css("display", "none");
        }
    });
    ...
    requestData(null,"/mediaPlate/0","get","json",false, function (data) {
        ...
        $(data).each(function (i, item) {
            ...
            if (item.standarPlatformFlag == 1) {
                standardHtml += "<div style='width: 10%;float: left;'><span style='white-space: nowrap;text-overflow: ellipsis;overflow: hidden;width: 100%;' class='btn btn-outline plateSpan' title='" + item.name + "' data-standarPlatformFlag='" + item.standarPlatformFlag + "' data-value='" + item.id + "' onclick='setType(" + item.id + ",this)'>" + item.name + "</span></div>";
            } else {
                notStandardHtml += "<div style='width: 10%;float: left;'><span style='white-space: nowrap;text-overflow: ellipsis;overflow: hidden;width: 100%;' class='btn btn-outline plateSpan' title='" + item.name + "' data-standarPlatformFlag='" + item.standarPlatformFlag + "' data-value='" + item.id + "' onclick='setType(" + item.id + ",this)'>" + item.name + "</span></div>";
            }
        });
        //判断标准和非标准是否有，控制隐藏显示
        if (standardHtml) {
        ...
        colModel: [
            ...
            },
            {
                ...
                formatter: function (v, options, row) {
                    ...
                    if(mediaChangePermissionFlag){
                        html += "<a class='text-success' onclick='mediaChangeObj.mediaChangeShow("+row.plateId+", "+row.id+");'>异动详情&nbsp;</a>";
                    }
                    return html;
                }
            },
    ...
    orderTable: {
        ...
        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
            ...
            },
            {
                ...
                sortable:false,
                formatter: function (v, options, row) {
                    return "<input style='text-align: center;' onkeydown='enterFun(event, \"num\", "+row.mediaId+", this)' onkeypress=\"return inNum(event)\" size=\"5\" value=\""+v+"\" onblur=\"updateNum("+row.mediaId+",this)\"/>";
                }
            },
            {
                name: 'price',
                ...
                sortable:false,
                formatter: function (v, options, row) {
                    return "<span class=\"text-red font-bold \">￥</span><input style='text-align: center;width: 100px;' onkeydown='enterFun(event, \"price\", "+row.mediaId+", this)'  onkeypress=\"return inNum(event)\" onblur='updatePrice("+row.mediaId+",this)' size=\"3\" maxlength=\"14\" value='"+v+"'/>";
                }
            },
            {
                name: 'amount',
...
//回车事件
function enterFun(evt, type, mediaId, t) {
...
function updatePrice(mediaId,t) {
    ...
    //计算单价
    orderTableObj.data[key].unitPrice = currentPrice / orderTableObj.data[key].discount * 100 / orderTableObj.data[key].num;
    calcuSum(); //计算总价
}
        ...
        colModel: [  //这里会根据index去解析jsonReader中root对象的属性，填充cell
            ...
            },
            {
                ...
                formatter: function (v, options, row) {
                    var html = "";
                    if(mediaChangePermissionFlag){
                        html += "<a class='text-success' onclick='mediaSupplierChangeObj.mediaSupplierChangeShow("+row.relateId+");'>异动详情&nbsp;</a>";
                    }
                    return html;
                }
            },
...
function batchExport() {
    ...
    for (var k in json) {
        //校验是否选择了下拉列表筛选项
        if(mediaTermSelectMap[$("#plateId").val()] && mediaTermSelectMap[$("#plateId").val()].contains(k) && json[k]){
            json.exportFlag = true;
        }
    ...
    if(!json.exportFlag){
        layer.msg("必须选择一个下拉类型的筛选条件才能进行导出操作！", {time: 3000, icon: 5});
        return;
    }
    ...
    json.plateName = $("#mediaTypeText").text();
    location.href = baseUrl+"/media1/batchExport?"+$.param(json);
}
    ...
    if(ids && ids.length > 0){
        ...
        $(ids).each(function (index, id) {
            ...
            if(row.companyCode != companyCode){
                info.push("<span style=\"color: brown;font-size: 12px;\">很抱歉，存在["+row.name+"]不能导出，只能导出自己公司的媒体异动！</span>")
...
var mediaChangeObj = {
    ...
    mediaChangeShow: function (plateId, mediaIds) {
        layer.open({
            type: 1,
            title: '媒体异动详情',
...
var mediaSupplierChangeObj = {
    ...
    mediaSupplierChangeShow: function (relateIds) {
        layer.open({
            type: 1,
            title: '媒体供应商异动详情',
            content: $("#mediaChange").html(),
