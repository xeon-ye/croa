 ...
 * 集团财务部门负责人
 */
public class JTCWdepartmentHeadler implements ICommonTaskHandler, TaskListener  {
    @Override
            ...
            switch (state){
                ...
                case IConst.STATE_JTCWBZ:
                    ...
                    if (!CollectionUtils.isEmpty(dept)){
                        ...
                        delegateTask.setVariable("acceptWorker",dept.get(0).getMgrId());
                        nextUser = dept.get(0).getMgrId().toString();
                        nextUserName = dept.get(0).getMgrName();
                    }
                    break;
                // 不存在；
                default:
