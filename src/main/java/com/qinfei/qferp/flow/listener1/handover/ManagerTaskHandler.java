...
public class ManagerTaskHandler implements TaskListener, ICommonTaskHandler {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        List<User> userList = userService.queryCWZJInfo(delegateTask.getVariable("companyCode", String.class));
        if(CollectionUtils.isEmpty(userList)){
            throw new QinFeiException(1002,"未获取到下一个审核人");
        }
        employeeConnect.setConFinanceId(userList.get(0).getId());
        employeeConnect.setConFinanceName(userList.get(0).getName());
