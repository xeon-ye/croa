...
/**
 * @CalssName ReportTaskHandler
 * @Description 填写出差总结报告
 * @Author xuxiong
 * @Date 2019/9/27 0027 16:01
 * @Version 1.0
 */
public class ReportTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
...
// 判断前端是否有审核人信息传递过来；
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            switch (state) {
                ...
                case IConst.STATE_REPORT:
                    ...
                    User user = userService.getById(initiatorWorker);
                    if (user != null) {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        if (map.get("acceptWorker") != null) {
            leave.setApproverUserId(String.valueOf(map.get("acceptWorker")));
        }
