...
public class ProcessFinishHandler implements TaskListener, ICommonTaskHandler {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        InvoiceMapper invoiceMapper = SpringUtils.getBean("invoiceMapper");
        //后处理稿件表的状态
        List<Article> articleList = invoiceMapper.queryArticleByIdAndState(Integer.parseInt((String)map.get("dataId")),IConst.FEE_STATE_PROCESS) ;
        if (!CollectionUtils.isEmpty(articleList) &&  state!=IConst.STATE_REJECT){
            Map<String,Object> map1 = new HashMap<>() ;
            map1.put("list",articleList) ;
            map1.put("state",IConst.FEE_STATE_FINISH) ;
            invoiceMapper.changeInvoiceState2(map1);
        }
