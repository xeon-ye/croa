 ...
 * @Description 政委领导审核
 * @Author xuxiong
 * @Date 2019/9/25 0025 10:21
 * @Version 1.0
 */
public class ZwTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            switch (state){
                ...
                case IConst.STATE_REJECT:
                    ...
                    break;
                case IConst.STATE_ZW:
                    DeptZwMapper deptZwMapper = SpringUtils.getBean(DeptZwMapper.class);
                    Integer initiatorDept = delegateTask.getVariable("initiatorDept", Integer.class);
                    if(initiatorDept != null){
                        List<User> userList = deptZwMapper.listUserByParam(null,initiatorDept);
                        if(CollectionUtils.isEmpty(userList)){
                            throw new QinFeiException(1002, "申请人部门没有绑定政委人员！");
                        }
                        delegateTask.setVariable("acceptDept", userList.get(0).getDeptId());
                        delegateTask.setVariable("acceptWorker", userList.get(0).getId());
                        nextUser = userList.get(0).getId() == null ? null : String.valueOf(userList.get(0).getId());
                        nextUserName = userList.get(0).getName();
                    }else {
                        throw new QinFeiException(1002, "申请人部门信息不存在！");
                    }
                    break;
                // 不存在；
                default:
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        if(state == IConst.STATE_REJECT){
            ...
            commonRejectHandle(delegateTask, (String)map.get("pictureAddress"), processName, (String)map.get("dataUrl"), map); //驳回处理方法
        }else {
            //待办消息分类
            map.put("parentType",1);//待办
            commonDefaultHandle(delegateTask, (String)map.get("pictureAddress"), processName, (String)map.get("dataUrl"), map); //默认其他情况处理方法
        }
