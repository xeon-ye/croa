package com.qinfei.qferp.flow.listener1.transfer;
...
/**
 * @CalssName TransferLeaderTaskHandler
 * @Description 转入部门审核
 * @Author xuxiong
 * @Date 2019/9/30 0030 10:20
 * @Version 1.0
 */
public class TransferLeaderTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
    public void setApproveUser(DelegateTask delegateTask, int state) {
        ...
        Integer afterDept = delegateTask.getVariable("afterDept",Integer.class);
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            switch (state) {
                ...
                break;
                case IConst.STATE_ZRBZ:
                    DeptMapper deptMapper = SpringUtils.getBean("deptMapper");
                    //获取转入部门，查询转入部门部长
                    Dept dept = deptMapper.getById(afterDept);
                    Integer userId =dept.getMgrId();
                    delegateTask.setVariable("acceptDept",dept.getId());
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","员工调岗");//消息子类类型
        map.put("type",18);//员工调岗
        ...
        employeeService.processTransfer(Integer.parseInt(delegateTask.getVariable("empId", String.class)), delegateTask.getVariable("code", String.class), state, taskId, itemId);
        String uniqueKey = PrimaryKeyUtil.getStringUniqueKey();   // 更新查询码；
        delegateTask.setVariable("code", uniqueKey);
        delegateTask.setVariable("dynamicUrl", "/transferApprove?code=" + uniqueKey); // 更新链接；
    }
