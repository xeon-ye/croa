...
public class ProcessCompleteHandler implements ICommonTaskHandler, TaskListener {
    @Override
    ...
    public void handleApproveData(DelegateTask delegateTask) {
        ...
        int state;
        if (agree) {
            state = IConst.PROTECT_STATE_FINISH;
        } else {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        // 完成插入企查查表
        if (state == IConst.PROTECT_STATE_FINISH) {
            standardizedCompanyService.updateCompany(delegateTask.getVariable("companyName", String.class));
        }
    }
