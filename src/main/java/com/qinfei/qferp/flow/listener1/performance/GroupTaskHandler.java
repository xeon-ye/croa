package com.qinfei.qferp.flow.listener1.performance;
...
/**
 * @CalssName GroupTaskHandler
 * @Description 考核小组评分
 * @Author xuxiong
 * @Date 2019/9/30 0030 8:54
 * @Version 1.0
 */
public class GroupTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","绩效");//消息子类类型
        map.put("type",16);//绩效
        ...
        performanceScoreService.processPerformance(Integer.parseInt((String) map.get("dataId")), delegateTask.getVariable("code", String.class), state, taskId, itemId);
        String uniqueKey = PrimaryKeyUtil.getStringUniqueKey(); // 更新查询码；
        delegateTask.setVariable("code", uniqueKey);
        delegateTask.setVariable("dynamicUrl", "/performanceApprove?code=" + uniqueKey); // 更新链接；
    }
