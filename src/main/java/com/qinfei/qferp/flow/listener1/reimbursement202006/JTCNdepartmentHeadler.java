 ...
 *
 */
public class JTCNdepartmentHeadler  implements ICommonTaskHandler, TaskListener {
    @Override
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            ...
            switch (state){
                ...
                case IConst.STATE_CN:
                    datas = getApproveUserId(delegateTask, IConst.ROLE_TYPE_CW, IConst.ROLE_CODE_CN, "JT", true);
                    nextUser = datas[0];
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        if(state == IConst.STATE_REJECT){
            ...
            List<User> list = userChuna.listByTypeAndCode(IConst.ROLE_TYPE_CW, IConst.ROLE_CODE_CN, "JT");
