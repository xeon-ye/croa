package com.qinfei.qferp.flow.listener1.productApply;
...
/**
 * @CalssName financeApprove
 * @Description 行政部长审核
 * @Author tsf
 */
public class financeApprove implements TaskListener, ICommonTaskHandler {
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            ...
            switch (state) {
                ...
                case IConst.STATE_XZ:
                    datas = getApproveUserId(delegateTask, IConst.ROLE_TYPE_XZ, IConst.ROLE_CODE_BZ, company, false);
                    nextUser = datas[0];
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","物品领用");//消息子类类型
        map.put("type",25);//物品领用
        ...
        //更新物品领用数据
        ReceiveApply apply = new ReceiveApply();
        apply.setId(Integer.valueOf(map.get("dataId").toString()));
        apply.setUpdateTime(new Date());
        apply.setItemId(itemId);
        apply.setTaskId(taskId);
        apply.setState(state);
        IApplyService applyService = SpringUtils.getBean("applyService");
        applyService.processApply(apply);
    }
}
