 ...
 * @Version 1.0
 */
public class ExecDirectorGateHandler implements ICommonTaskHandler, TaskListener {
    @Override
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","加班");//消息子类类型名称
        map.put("type",8);//加班
        ...
        }else if(state == IConst.STATE_FINISH){
            commonFinishHandle((String)map.get("pictureAddress"), (String)map.get(IProcess.PROCESS_NAME), (String)map.get("dataUrl"), map);
        }else {
            //待办消息分类
