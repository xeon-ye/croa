package com.qinfei.qferp.flow.listener1.manuscript;
...
/**
 * @CalssName CashierTaskHandler
 * @Description 出纳审核
 * @Author xuxiong
 * @Date 2019/9/29 0029 15:47
 * @Version 1.0
 */
public class CashierTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","退稿");//消息子类类型
        map.put("type",6);//退稿
        ...
        //更新数据
        Drop drop = new Drop();
        drop.setId(Integer.parseInt((String) map.get("dataId")));
        drop.setState(state);
        drop.setUpdateUserId(AppUtil.getUser().getId());
        // 更新流程当前的任务ID；
