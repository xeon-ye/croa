package com.qinfei.qferp.flow.listener1.leave;
...
public class ProcessFinishHandler implements TaskListener, ICommonTaskHandler {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","员工离职");//消息子类类型
        map.put("type",20);//员工离职
        if(state == IConst.STATE_REJECT){
            //消息分类
        ...
        employeeService.processLeave(Integer.parseInt((String) map.get("dataId")), delegateTask.getVariable("code", String.class), state, taskId, itemId);
        String uniqueKey = PrimaryKeyUtil.getStringUniqueKey();  // 更新查询码；
        delegateTask.setVariable("code", uniqueKey);
        delegateTask.setVariable("dynamicUrl", "/leaveApprove?code=" + uniqueKey); // 更新链接；
    }
}
