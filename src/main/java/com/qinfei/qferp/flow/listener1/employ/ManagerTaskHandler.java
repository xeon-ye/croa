package com.qinfei.qferp.flow.listener1.employ;
...
/**
 * @CalssName ManagerTaskHandler
 * @Description 总经理审核
 * @Author xuxiong
 * @Date 2019/9/27 0027 15:15
 * @Version 1.0
 */
public class ManagerTaskHandler implements TaskListener, ICommonTaskHandler {
    @Override
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            ...
            switch (state) {
                ...
                case IConst.STATE_CEO:
                    datas = getApproveUserId(delegateTask, IConst.ROLE_TYPE_ZJB, IConst.ROLE_CODE_ZJL, company, false);
                    nextUser = datas[0];
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        map.put("messageTypeName","员工录用");//消息子类类型
        map.put("type",21);//员工录用
        ...
        entryService.processEntry(Integer.parseInt((String) map.get("dataId")), delegateTask.getVariable("code", String.class), state, taskId, itemId, getOpinion(delegateTask), delegateTask.getVariable("desc", String.class));
        String uniqueKey = PrimaryKeyUtil.getStringUniqueKey(); // 更新查询码；
        delegateTask.setVariable("code", uniqueKey);
        delegateTask.setVariable("dynamicUrl", "/employApprove?code=" + uniqueKey);  // 更新链接；
    }
}
