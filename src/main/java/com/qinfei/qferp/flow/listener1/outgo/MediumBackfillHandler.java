 ...
 * 媒介回填
 */
public class MediumBackfillHandler  implements ICommonTaskHandler, TaskListener {
    @Override
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)) {
            ...
            switch (state){
                ...
                case IConst.STATE_REJECT:
                    ...
                    break;
                case IConst.STATE_MEDIUMBACKFILL:
                    String userId= String.valueOf(delegateTask.getVariable("userId"));
                    String userName= String.valueOf(delegateTask.getVariable("userName"));
                    nextUser = userId;
    ...
    public void handleApproveData(DelegateTask delegateTask) {
        ...
        int state;
        if (agree) {
            state = IConst.STATE_MEDIUMBACKFILL;
        } else {
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        if(state == IConst.STATE_REJECT){
            ...
            // 获取媒介回填的链接页面；
            String backUrl = delegateTask.getVariable(IProcess.PROCESS_BACKFILL_URL, String.class);
            if (StringUtils.isEmpty(backUrl)) {
                backUrl = (String) map.get("backUrl");
            }
            // 处理中的待办事项跳转到流程审核页面；
            map.put("transactionAddress", backUrl);
            ...
            User users = userService.getById(Integer.parseInt(String.valueOf(delegateTask.getVariable("userId"))));
...
//            User chuna = list.get(0);
            delegateTask.setVariable("acceptDept", users.getDeptId());
            delegateTask.setVariable("acceptWorker", users.getId());
        }
