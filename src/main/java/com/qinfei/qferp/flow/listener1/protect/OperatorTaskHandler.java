 ...
 * 销售
 */
public class OperatorTaskHandler implements ICommonTaskHandler, TaskListener {
    @Override
        ...
        if (StringUtils.isEmpty(nextUser) || StringUtils.isEmpty(nextUserName) || StringUtils.isEmpty(nextUserDept)){
            ...
            switch (state){
                ...
                case IConst.STATE_REJECT:
                    ...
                    break;
                case IConst.PROTECT_STATE_OPERATOR:
    ...
    public void updateProcessData(DelegateTask delegateTask) {
        ...
        //更新数据
        CrmCompanyProtect protect = new CrmCompanyProtect();
        protect.setId(Integer.parseInt((String)map.get("dataId")));
    ...
    public void handleApproveData(DelegateTask delegateTask) {
        ...
        int state;
        if (agree) {
            state = IConst.PROTECT_STATE_OPERATOR;
        } else {
